...
The PostgreSQL cutover starts a new Data Epoch. Counts, Current State, Aggregates, Definitions, and
Item-improvement Facts are not continuous with the MongoDB epoch.
...
Both backends must populate all 18 keys. The existing MongoDB history uses the stable epoch ID
`legacy-mongodb` with `startedAt: null` because its true beginning is not known. PostgreSQL uses the
explicit UUID and timestamp stored in `data_epochs`.

Persist exactly one Data Epoch in a small PostgreSQL control table. An explicit deployment command
creates it with a UUID and cutover timestamp after preflight validation and before production traffic.
Application startup must fail if the active PostgreSQL database has zero or multiple epoch rows.
Starting another Data Epoch requires a new database rather than mixing histories in the same tables.
Include the same epoch ID and start timestamp in Community Dump manifests and item-improvement export
responses so consumers can detect the continuity boundary.
...
| --- | --- | --- |
| `singleton` | `boolean` | Primary key, default `true`, check that the value is true. |
| `id` | `uuid` | Not null and unique; generated by the explicit epoch command. |
| `started_at` | `timestamptz` | Not null; operator-selected cutover instant. |
| `created_at` | `timestamptz` | Not null, default `clock_timestamp()`. |

`id` is unique. The singleton primary key enforces exactly one possible epoch row per database.
...
| `epoch_id` | `uuid` | Not null; references `data_epochs(id)`. |
...
Add a unique constraint on `(epoch_id, dump_month, schema_version)`.
...
must require one exact `data_dump_runs.id`, verify that run's epoch, Dump Month, schema version,
...
- An additive `epoch` object matching `/api/status.database.epoch`.
...
| v3 item-improvement export | Both backends cover limit clamping, origin omission, numeric timestamps, public `_id`, cursor canonicalization, epoch, ordering, and empty pages; PostgreSQL additionally covers database-time settled pagination. |
...
| `GET /api/report/v3/item_improvement_recipes/availability` | Filters, clamp, public `_id`, epoch, cursor, omission of origins, and cache headers match; PostgreSQL alone applies the settled window. |
...
| `GET /api/status` | Returns `env`, `disk`, and the exact backend-neutral `database` shape with all 18 estimated counts and Data Epoch metadata. |
...
2. **Shared contracts**
   ...
   - Add backend-neutral Data Epoch and `/api/status.database` response schemas.
...
3. **Mongo action split**
   ...
   - Preserve MongoDB persistence behavior except for the explicitly documented shared validation,
     status, and epoch response changes.
...
3. On the PostgreSQL machine, provision a disposable preflight database, run migrations, create a test
   epoch, and run the full PostgreSQL HTTP behavior and load tests against it.
...
6. Immediately before cutover, create the sole production Data Epoch, start poi-server, and verify
   startup/schema/status health without synthetic production writes.
...
The traffic switch is best effort rather than a strict transactional epoch boundary. In-flight
