...
function getNewVersionDownloadLink(version, target) {
  const pure = version.substring(1)
  switch (target) {
    case 'linux-x64':
    ...
    case 'linux-rpm-x64':
      return `${BASE_URI}/${version}/poi-${pure}.rpm`
    case 'macos-x64':
  ...
  render() {
    ...
    return (
      <div className={styles.container}>
        ...
        <a href={(semver.gte(version.stable, 'v7.8.0-beta.2') ? getNewVersionDownloadLink : getDownloadLink)(version.stable, target)}>
        ...
        { version.betaAvailable &&
          <a href={(semver.gte(version.beta, 'v7.8.0-beta.2') ? getNewVersionDownloadLink : getDownloadLink)(version.beta, target)}>
